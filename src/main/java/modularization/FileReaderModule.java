...
public class FileReaderModule extends ModuleImpl {
	...
	public FileReaderModule(CallbackReceiver callbackReceiver, Properties properties) throws Exception {
		...
		this.getPropertyDescriptions().put(PROPERTYKEY_USEGZIP, "Set to 'true' if the input file is compressed using GZIP");
		this.getPropertyDescriptions().put(PROPERTYKEY_ENCODING, "The text encoding of the input file (if applicable, else set to empty string)");
		...
		this.getPropertyDefaultValues().put(PROPERTYKEY_INPUTFILE, homedir + fs + "input.txt");
