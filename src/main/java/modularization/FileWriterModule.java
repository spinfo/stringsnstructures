...
public class FileWriterModule extends ModuleImpl {
	...
	public FileWriterModule(CallbackReceiver callbackReceiver, Properties properties) throws Exception {
		...
		this.getPropertyDescriptions().put(PROPERTYKEY_USEGZIP, "Set to 'true' if the output file is to be compressed using GZIP");
		this.getPropertyDescriptions().put(PROPERTYKEY_ENCODING, "The text encoding of the output file (if input is a char pipe)");
	...
	public boolean process() throws Exception {
		...
		try {
			...
			try {
				...
				OutputStream fileOutputStream = new FileOutputStream(new File(this.filePath));
				...
				// Log message
				Logger.getLogger(this.getClass().getSimpleName()).log(Level.INFO, "Wrote byte stream input into "+this.filePath);
				...
				Writer fileWriter = new OutputStreamWriter(fileOutputStream, this.encoding);
				...
				// Log message
				Logger.getLogger(this.getClass().getSimpleName()).log(Level.INFO, "Wrote character input to "+this.filePath);
			}
