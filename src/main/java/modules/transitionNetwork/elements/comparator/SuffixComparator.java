...
public class SuffixComparator implements Comparator<SuffixElement>{
	...
	@Override public int compare(SuffixElement e1,SuffixElement e2){
		int i=e1.start;
		int j=e2.start;
		while ((i<e1.end)&&(j<e2.end)) 
		{
			if(this.text[i] > this.text[j]) return 1;
			else if (this.text[i] < this.text[j]) return -1;
			i++;j++;
		}
		if (i<e1.end) return -1;
		else if (j<e2.end) return 1;
		return 0;
