...
public class KeyWordInPhrase {
	...
	static final String ENCODING = "UTF-8";//StandardCharsets.UTF_8;	
	static final String TXTEXTENSION=".txt";
	static String PATH=pathName(); 
	static String name=ReadNameOfInputFile(PATH);	
	static String fileSeparator = System.getProperty("file.separator");
	static String INPUT_FILE_NAME = PATH+name+"Preprocess"+TXTEXTENSION;
	static String OUTPUT_RESULT_FILE_NAME = PATH+name+"Kwip"+TXTEXTENSION;;
	static String OUTPUT_TYPE_FILE_NAME = PATH+name+"Kwip"+"Type"+TXTEXTENSION;;
	static String OUTPUT_UNIT_FILE_NAME = PATH+name+"Kwip"+"Unit"+TXTEXTENSION;;
	...
	private static String ReadNameOfInputFile(String PATH){
		...
		try {BufferedReader reader = 
				new BufferedReader
				(new FileReader(PATH+"Name"+TXTEXTENSION));
	...
	static StringBuffer prettyBuf=
	new StringBuffer("<HTML><HEAD><meta charset=\"utf-8\"><TITLE> </TITLE></HEAD><BODY>");
	
	
	
	private static void addDoc(IndexWriter w, String token, String context, int line) throws IOException 
	{
		...
		try
		{
			...
			try {
			    ...
			    while ((text = bufreader.readLine()) != null) {
			    	System.out.println(String.valueOf(lineNr) + " "+text);
			    	// each token is one document
			    	List<String> tokenList=
			    	TokenizeString.tokenizeString(analyzer,text);
	...
	static void tokens(IndexReader reader,String querystr) {
		...
		try{
			...
			Query query = 
			new QueryParser
			(Version.LUCENE_46, "Text", analyzer).parse(querystr);
			...
			for(int i=0;i<hits.length;++i) {
				...
				System.out.println("tokens querystr: "+
				querystr+ " context: "+ context /*d.get("Context")*/ +" source: "+
				d.get("Source"));
				...
				System.out.println("tokens "+e.getMessage());
				e.printStackTrace();
				int x1 = 10/0;
				 System.exit(3);
			}	   
	...
	static void writeToFileUnitList(ArrayList<Integer> unitList){
		/* a unit is the number of tokens for a type; 
		 * a unit is defined as the last token of a type. Its value is a int,
		 * counting from the last token of the first type to the next last token
		 * of the next type (e.g. 3 (if there are three tokens of the first type),
		 * 5 (if there are two tokens of the second type), ...
		 */
	...
	static void writeToFileTypeList(ArrayList<String> typeList){
		...
		try
	       ...
	       {System.out.println("writeToFileTypeList IOException: "+e);
	       e.printStackTrace();
	       int x1 = 10/0;
	       System.exit(5);
	       }
	...
	static void types(Directory index,IndexReader reader) {	
		...
		try {						
			...
			while (termEnum.next()!=null) { 
				...
				if (type.length()<8) len=2; else len=1;
				System.out.println("types type: "+type+tabs.substring(0,len)
		...
		public static void main(String[] args)
		{
			...
			{		
			    ...
			    types(index,reader);		   			 				
			    // reader can only be closed if there is no need to access the documents any more
			    reader.close();
			    ...
			    System.out.println("vor prettyWriter");
			    FileWriter prettyWriter = 
			    new FileWriter(PATH+name+"PrettyKwip"+".html");    
			    
			    prettyWriter.write(prettyBuf.toString());
			   // System.out.println(prettyBuf);
			    prettyWriter.close();
			    System.out.println("nach prettyWriter");
			}
			catch(Exception e){ 
			System.out.println(e.getMessage());
			e.printStackTrace();
